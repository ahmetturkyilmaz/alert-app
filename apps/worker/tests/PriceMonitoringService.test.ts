...
describe("PriceMonitoringService", () => {
    ...
    beforeEach(() => {
        binanceService = {fetchBinancePrices: jest.fn()} as unknown as jest.Mocked<BinanceService>;
        notificationService = {getTriggeredNotifications: jest.fn()} as unknown as jest.Mocked<NotificationService>;
        sqsService = {sendNotification: jest.fn().mockResolvedValue(undefined)} as unknown as jest.Mocked<SQSService>;

        priceMonitoringService = new PriceMonitoringService(binanceService, notificationService, sqsService);
    });
    ...
    it("should fetch prices and send notifications when conditions match", async () => {
        ...
        notificationService.getTriggeredNotifications.mockResolvedValue(mockNotifications);

        await priceMonitoringService.processPrices();

        expect(binanceService.fetchBinancePrices).toHaveBeenCalled();
        expect(notificationService.getTriggeredNotifications).toHaveBeenCalledWith(mockPrices);
    ...
    it("should handle errors in processPrices gracefully", async () => {
        ...
        expect(consoleSpy).toHaveBeenCalledWith("Error fetching or processing data:", "Binance API failure");
