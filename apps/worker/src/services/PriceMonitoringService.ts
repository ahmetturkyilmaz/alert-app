...
export class PriceMonitoringService {
    ...
    async processPrices(): Promise<void> {
        try {
            ...
            const notificationsToTrigger = await this.notificationService.getTriggeredNotifications(fetchedPrices);

            for (const notification of notificationsToTrigger) {
                this.sqsService
                    .sendNotification(process.env.AWS_SQS_NOTIFICATION_ARN || "", notification)
                    .catch(console.error);
            }
        } catch (error) {
